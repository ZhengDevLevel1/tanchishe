...
function Snake({ playerId, color, isLocal }: { playerId: string, color: string, isLocal: boolean }) {
  ...
  useFrame((state, delta) => {
    ...
    // Scale based on score (infinitely wider)
    const scale = Math.max(1, Math.pow(player.score / 10, 0.4));
  ...
  return (
    <group>
      ...
      <instancedMesh ref={bodyRef} args={[null as any, null as any, 15000]} castShadow receiveShadow frustumCulled={false}>
...
function Orbs() {
  ...
  return (
    <instancedMesh ref={meshRef} args={[null as any, null as any, 3000]} castShadow receiveShadow frustumCulled={false}>
...
export function GameScene() {
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (e: PointerEvent) => {
      if (e.button === 0 || e.pointerType === 'touch') inputs.current.boost = true;
    };

    const handlePointerUp = (e: PointerEvent) => {
      if (e.button === 0 || e.pointerType === 'touch') inputs.current.boost = false;
    };
  ...
  useFrame((state, delta) => {
    ...
    if (serverPlayer && serverPlayer.state === 'alive') {
      ...
      // Local movement logic: steer towards mouse
      ...
      const targetAngle = Math.atan2(worldPos.y - head.y, worldPos.x - head.x);
      let diff = targetAngle - localPlayerRef.current.currentAngle;
      ...
      if (Math.abs(diff) > maxTurn) {
        ...
        localPlayerRef.current.currentAngle += diff;
      }
      ...
      const myScale = Math.max(1, Math.pow(localPlayerRef.current.score / 10, 0.4));
      
      const prev = localPlayerRef.current.segments.length > 1 ? localPlayerRef.current.segments[1] : localPlayerRef.current.segments[0];
      const dxSq = (head.x - prev.x) ** 2 + (head.y - prev.y) ** 2;
      const spacing = 0.5 * myScale;
      if (dxSq >= spacing * spacing) {
        localPlayerRef.current.segments.unshift(head);
      } else {
        localPlayerRef.current.segments[0] = head;
      }

      const targetLength = Math.floor(localPlayerRef.current.score / myScale);
      while (localPlayerRef.current.segments.length > Math.max(10, targetLength)) {
      ...
      for (const otherId in gs.players) {
        ...
        const otherScale = Math.max(1, Math.pow(other.score / 10, 0.4));
        const hitDist = (0.6 * myScale + 0.6 * otherScale) * 0.8;
      ...
      if (now - localPlayerRef.current.lastSendTime > 50) {
        sendPlayerState({
          segments: localPlayerRef.current.segments.map(s => ({
      ...
      // Dynamic zoom
      const targetZ = Math.min(120, Math.max(40, 40 + (localPlayerRef.current.score - 10) * 0.8));
